...
- **Don't rename `proxmox_virtual_environment_*` → `proxmox_*`** in the OpenTofu files. The shorter aliases are a different schema (no `disk`/`network_device`/`serial_device` nested blocks). Deprecation warnings on the old names are harmless until bpg v1.0.
